...
<p><a href="https://gorunner.pro" style="background:#f97316;color:#fff;padding:8px 20px;border-radius:6px;text-decoration:none;font-weight:600;">Go to RUNNER</a></p>`,
  ...
  // POST /runner/availability — set agent/vendor availability windows
  if (subpath === 'availability' && method === 'POST') {
    const body = await request.json().catch(() => ({}));
    const userId = (body.user_id || '').trim();
    if (!userId) return json({ error: 'user_id required' }, 400);
    const slots = body.slots;
    if (!Array.isArray(slots)) return json({ error: 'slots array required' }, 400);
    await kv.put(`runner:availability:${userId}`, JSON.stringify({
      user_id: userId, slots, updated_at: new Date().toISOString(),
      timezone: body.timezone || 'America/New_York',
    }));
    return json({ success: true, user_id: userId, slots });
  }

  // GET /runner/availability?user_id=X
  ...
  // GET /runner/calendar?user_id=X&date=2026-03-11&range=week
  if (subpath === 'calendar' && method === 'GET') {
    const userId = url.searchParams.get('user_id') || '';
    ...
    if (range === 'day') {
      ...
      rangeEnd = dateStr.slice(0, 8) + String(lastDay).padStart(2, '0');
    } else {
